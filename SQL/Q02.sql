...
FROM (
	SELECT t_milk.industry_name, t_milk.rok, 
		round((t_milk.average_value / t_milk.average_price)) AS l_milk_2006,
		round((t_bread.average_value / t_bread.average_price)) AS kg_bread_2006
	FROM (
	SELECT industry_name, rok, average_value, product_name, average_price
		...
		WHERE rok = (
			SELECT min(rok) 
			FROM t_jan_visnak_project_sql_primary_final )
			AND product_name LIKE 'Mléko%') t_milk
	JOIN (
		SELECT industry_name, rok, average_value, product_name, average_price
		FROM t_jan_visnak_project_sql_primary_final 
		WHERE rok = (
			SELECT min(rok) 
			FROM t_jan_visnak_project_sql_primary_final )
		AND product_name LIKE 'Chléb%') t_bread
	ON t_milk.industry_name = t_bread.industry_name) table_1
JOIN (
	SELECT t_milk.industry_name, t_milk.rok, 
		round((t_milk.average_value / t_milk.average_price)) AS l_milk_2018,
		round((t_bread.average_value / t_bread.average_price)) AS kg_bread_2018
	FROM (
		SELECT industry_name, rok, average_value , product_name, average_price
		FROM t_jan_visnak_project_sql_primary_final 
		WHERE rok = (
	...
	JOIN (
		...
		WHERE rok = (
			...
			FROM t_jan_visnak_project_sql_primary_final)
		AND product_name LIKE 'Chléb%') t_bread
	ON t_milk.industry_name = t_bread.industry_name) table_2
